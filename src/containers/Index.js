...
const ListPosts = ({ posts }) => {
  const recent_posts = posts.sort(byDate);

  return (
    <aside>
      <ol id="recent-posts" className="posts">
  ...
  return (
    <React.Fragment>
      ...
      <div className="container">
        <section id={"index-content"}>{convert(html)}</section>
        <h3>Posts</h3>
        <ListPosts posts={meta.posts} />
      </div>
    </React.Fragment>
  );
};
...
export default () => (
  <RouteData
    render={routeData => (
      <SiteData
        render={siteData => <Index {...{ ...routeData, siteData }} />}
      />
    )}
  />
);
